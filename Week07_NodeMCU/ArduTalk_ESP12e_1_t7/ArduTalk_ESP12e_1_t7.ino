...
///
////////// This is a modified version, t7; by tsaiwn@cs.nctu.edu.tw
...
#include <Arduino.h>
const String version = "ESP12e_1_t7";

/// Comment out next LINE if using NodeMCU V3
#define NODE_V2 1   //  NodeMCU V2  :  D0 (GPIO16) connect with 2nd onBoard LED

#define AP_IP_LAST_DIGIT  1    // 192.168.0.1   ;  Last digit, Default is .1 for AP server 
...
//#define DEBUG_ESP 1  //  enable ESP Debuggin
//#define DEBUG 1     // Serial print some DEBUG info.
//#define DEBUG_PUSH_PULL 1   // print PUSH/PULL header MSG
//#define DEBUG_REGTIME 1
//#define DEBUG_REG_PUSH_PULL 1   // dummy register/push/pull
//#define DEBUG_DUMMY_STA 1   // dummy STA connected
//#define DEBUG_LOOP_CNT 1  // show LOOP count in loop( )
//#define DEBUG_FREE_MEM 1   // Report Free Heap Memory
    ...
    if (sw == 0){   // Not enforce via software(不是軟體強制), 若要強制CLEAR可傳不是 0 進來
        ...
        for(int i=0; i < 3; ++i) {   // divided into serval period to check button Released
          ...
          if( digitalRead(clr_ROM_pin) == HIGH)  {  // pin released (放掉了?)
             digitalWrite(16, ! digitalRead(16) );   /// by tsaiwn for NodeMCU V2, GPIO 16 = D0
...
void wifi_setting(void){  
 ...
 int ip_4th = 1;
 #ifdef AP_IP_LAST_DIGIT
    ip_4th = AP_IP_LAST_DIGIT;
 #endif
    IPAddress ip(192,168,0, ip_4th);            /////////  192.168.0.1  // ?  192.168.0.3; 6; 7
    IPAddress gateway(192,168,0, ip_4th);      /////////  192.168.0.1  // ?  192.168.0.3; 6; 7
...
#ifdef DEBUG_DUMMY_STA
   Serial.print("Dummy connected to "); Serial.println(String(wifiSSID));
   wifimode = 0;   // STA
   if(38 == 38) return;
#endif
  Serial.print("Try to connect to "); Serial.print(String(wifiSSID));
  Serial.print(" with PASSWD "); Serial.print(String(wifiPASS)); Serial.println( );
...
int iottalk_register(void){
...
#ifdef DEBUG
   Serial.println( );
   Serial.print("====== do  iottalk_register(void){ }  ...");
   Serial.println( );
#endif
#ifdef DEBUG_REG_PUSH_PULL
  ...
  reg_end = millis( );
   Serial.println( );
   Serial.print("===Reg start:"); Serial.println(reg_sta);
   Serial.print("         mid:"); Serial.println(reg_mid);
   Serial.print("         END:"); Serial.println(reg_end);
#endif
#ifdef DEBUG
    Serial.println("[HTTP] Register... code: " + (String)httpCode );
    Serial.println(http.getString());
    Serial.print("====== REGISTER DONE :  iottalk_register(void) ======");
    Serial.println( );
#endif

    //http.end();
    url +="/";  
    // if( httpCode == -1 ) httpCode = 200;  ///  ???
    // if( httpCode == -11 ) httpCode = 200;  ///  ???
    if( httpCode != 200) {
      digitalWrite(D0, LOW); delay(3); digitalWrite(D0, HIGH); delay(3);
    }
    return httpCode;
} // iottalk_register(void)
...
#ifdef DEBUG_PUSH_PULL
   Serial.print("Pull "); Serial.print(df_name);  Serial.println( );
#endif
#ifdef DEBUG_REG_PUSH_PULL
   if(38 == 38) return "-38-";
#endif
...
///////////////
 ...
 //   analogWriteFreq( 5000 );
...
#ifdef NODE_V2
    Serial.println( );
    Serial.print("=== ===  for V2 V2 V2 !!!  === ==="); Serial.println( );
#else
    Serial.println( );
    Serial.print("------   for V3   ------"); Serial.println( );
#endif
#ifdef DEBUG
     Serial.println( "DEBUG On "); Serial.flush( );
#endif
#ifdef DEBUG_REG_PUSH_PULL
     Serial.println( "DEBUG_REG_PUSH_PULL On -- dummy register/pull/push "); Serial.flush( );
#endif
#ifdef DEBUG_DUMMY_STA
     Serial.println( "DEBUG_DUMMY_STA  On -- dummy WiFi connected "); Serial.flush( );
#endif
    ...
    Serial.print(String(DefaultMyAP_SSID_));  Serial.println( ); Serial.flush( );
   Serial.print(" . . . . . . , IoTtalk Server :");
    Serial.print(String( IoTtalkServerIP ));  Serial.println( ); Serial.flush( );
 #ifdef DEBUG
...
// pull("D0~")  -->  analogWrite to GPIO 16
        int pinD0 = 16;
        pinD0 = 16;   // for NodeMCU V3 is OK
#ifdef NODE_V2
        pinD0 = 15;    // GPIO 15 == D8  ;;  for NodeMCU V2
#endif
        result = pull("D0~");
        if (result != "___NULL_DATA___"){
            Serial.print ("D0~: ");  Serial.print(result); Serial.println ( );
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(pinD0, result.toInt());  // 16,15
        }
        delay(2); 
// pull("D1~")  -->  analogWrite to GPIO 5
        result = pull("D1~");
        if (result != "___NULL_DATA___"){
            Serial.print ("D1~: ");  Serial.print(result); Serial.println ( );
...
// pull("D2~")  -->  analogWrite to GPIO 4
        result = pull("D2~");
        if (result != "___NULL_DATA___"){
            Serial.print ("D2~: ");  Serial.print(result); Serial.println ( );
...
// pull("D5")  -->  digitalWrite to GPIO 14
        result = pull("D5");
        if (result != "___NULL_DATA___"){
            Serial.print ("D5: ");  Serial.print(result); Serial.println ( );
...
// pull("D6")  -->  digitalWrite to GPIO 12
        result = pull("D6");
        if (result != "___NULL_DATA___"){
            Serial.print ("D6: ");  Serial.print(result); Serial.println ( );
...
// pull("D7")  -->  digitalWrite to GPIO 13      
        result = pull("D7");
        if (result != "___NULL_DATA___"){
            Serial.print ("D7: ");  Serial.print(result); Serial.println ( );
            tmp7 = 1023 * result.toFloat( );
            analogWrite(D7, int(tmp7) );
...
#ifndef NODE_V2
    ///  it is V3     ;;;  if NodeMCU V2, D8 is used as D0 for analogWrite( )
        result = pull("D8");
        if (result != "___NULL_DATA___"){
            Serial.print ("D8: ");  Serial.print(result); Serial.println ( );
            tmp7 = 1023 * result.toFloat( );
            analogWrite(D8, int(tmp7) );   //  D8 == GPIO 15
...
/// See: https://www.instructables.com/id/ESP8266-Using-GPIO0-GPIO2-as-inputs/
  ...
  reportMemory( );
    ...
    if (!LEDhadFlashed){    //  if need to flash / ON the LED
...
#ifdef  NODE_V2
      digitalWrite(D0, 0);    ///  ON ; another on board LED for NodeMCU V2
      delay(1);
      digitalWrite(D0, 1);   /// Off
#endif
        LEDisON = 0;
#ifdef DEBUG
        Serial.print("\n=== LED already ON for ");
